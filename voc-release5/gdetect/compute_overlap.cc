...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  // clip detection window to image boundary if the bbox is less than 70%
  // of the width and less than 70% of the height
//  const bool im_clip = ((double)(bbox_x2 - bbox_x1 + 1) / im_size_x < 0.7) &&
//                       ((double)(bbox_y2 - bbox_y1 + 1) / im_size_y < 0.7);

  const bool im_clip = false;
