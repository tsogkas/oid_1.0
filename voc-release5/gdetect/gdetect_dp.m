...
if time
    tic; model = filter_responses(model, pyra); toc;
else
...
%     r = fconv(pyra.feat{level}, filters, 1, length(filters));
    r = fconvsse_ST(pyra.feat{level}, filters, 1, length(filters)); % single-threaded version used for comparison
  else
