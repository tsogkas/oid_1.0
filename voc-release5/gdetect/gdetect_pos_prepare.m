function [pyra, model_dp] = gdetect_pos_prepare(im, model, boxes, fg_overlap, useUpperBound)
...
%   useUpperBound Use hierarchical filter tree cascade

if nargin < 5, useUpperBound = false; end
...
if useUpperBound
    model_dp = gdetect_dp_ub(pyra, model);
else
    model_dp = gdetect_dp(pyra, model);
end
