function model = jar_train_from_em_hog_shape_and_im_clusters(cls, root_only, num_clusters, note)
% Train a model.
%
...
try
  load([cachedir cls '_em_hog_shape_and_im_cluster_facing_pos']);
catch
  [em_sol, sel_inds] = em_hog_part_shape_and_im(pos, num_clusters);
  spos = em_hog_split_pos(pos(sel_inds), em_sol);
  save([cachedir cls '_em_hog_shape_and_im_cluster_facing_pos'], 'spos', 'em_sol', 'sel_inds');
end
%pos = cat(2, spos{:});
n = length(spos);
%for i = 1:n
...
try
  load([cachedir cls '_em_hog_shape_and_im_init1']);
catch
  seed_rand();
  for i = 1:n
    ...
    models{i} = train(models{i}, spos{i}, neg_small, true, true, 1, 1, ...
                      ...
                      ['em_hog_shape_and_im_init1_' num2str(i)]);
  end
  save([cachedir cls '_em_hog_shape_and_im_init1'], 'models');
end
...
try
  load([cachedir cls '_em_hog_shape_and_im_init2']);
catch
  seed_rand();
  for i = 1:n
    models{i} = train(models{i}, spos{i}, neg_small, false, false, 4, 3, ...
                      max_num_examples, fg_overlap, 0, false, ...
                      ['em_hog_shape_and_im_init2_' num2str(i)]);
  end
  save([cachedir cls '_em_hog_shape_and_im_init2'], 'models');
end
...
try 
  load([cachedir cls '_em_hog_shape_and_im_mix']);
catch
  seed_rand();
  ...
  if root_only
    model = train(model, pos, neg_small, false, false, 6, 5, ...
                  max_num_examples, fg_overlap, num_fp, false, 'em_hog_shape_and_im_mix1');
    model = train(model, pos, neg_large, false, false, 1, 5, ...
                  max_num_examples, fg_overlap, num_fp, true, 'em_hog_shape_and_im_mix2');
  else
    model = train(model, pos, neg_small, false, false, 1, 5, ...
                  max_num_examples, fg_overlap, num_fp, false, 'em_hog_shape_and_im_mix');
  end
  save([cachedir cls '_em_hog_shape_and_im_mix'], 'model');
end
...
if ~root_only
  ...
  % and hard negatives
  try 
    load([cachedir cls '_em_hog_shape_and_im_parts']);
  catch
    seed_rand();
    ...
    model = train(model, pos, neg_small, false, false, 8, 10, ...
                  max_num_examples, fg_overlap, num_fp, false, 'em_hog_shape_and_im_parts_1');
    % Finish training by data mining on all of the negative images
    model = train(model, pos, neg_large, false, false, 1, 5, ...
                  max_num_examples, fg_overlap, num_fp, true, 'em_hog_shape_and_im_parts_2');
    save([cachedir cls '_em_hog_shape_and_im_parts'], 'model');
  end
end

save([cachedir cls '_em_hog_shape_and_im_final'], 'model');

% test
anno = jarLoadAnno();
[ap, recall, prec] = jar_test(model, anno, 'aeroplane', 'test', true, '_em_hog_shape_and_im');
disp(ap);
