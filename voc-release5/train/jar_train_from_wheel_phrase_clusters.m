function model = jar_train_from_wheel_phrase_clusters(root_only, note)
% Train a model.
%
...
try
  load([cachedir 'wheelPhrase_clusters_facing']);
catch
  wheels_pos = jar_data('wheel', 'aeroplane', {'train','val'}, true);
  anno = jarLoadAnno();
  [~, wheel_phrase_inds] = cluster_wheel_phrases(pos, wheels_pos, anno, 10);
end
I = find(cellfun(@length, wheel_phrase_inds) > 30);
for i = 1:length(I)
  spos{i} = pos(wheel_phrase_inds{I(i)});
end
n = length(spos);
