...
% BASE_DIR    = './output';
BASE_DIR    = oidconf.paths.dataDirectory;
...
% PROJECT     = 'rbg/cvpr13-results/aspect-ratio-clustering-k-6';
...
% conf = cv(conf, 'pascal.dev_kit', [conf.paths.base_dir '/VOCdevkit_' ...
%                                    conf.pascal.year '/']);
...
conf = cv(conf, 'paths.model_dir', [oidconf.paths.modelsDirectory '/']);
...
conf.training.log = @(x) sprintf([oidconf.paths.logsDirectory '/%s.log'], x);
...
% JAR configuration
% -------------------------------------------------------------------
conf = cv(conf, 'jar.train_scale', 0.5);
conf = cv(conf, 'jar.test_scale', 0.5);
conf = cv(conf, 'em_hog.verticalStabilizer.tpt_size', 20);
conf = cv(conf, 'em_hog.verticalStabilizer.x_space', -0.5:0.5:0.5);
conf = cv(conf, 'em_hog.verticalStabilizer.y_space', -0.5:0.5:0.5);
conf = cv(conf, 'em_hog.verticalStabilizer.s_space', [0.6 0.5 0.7]);
% nose
conf.em_hog.nose = conf.em_hog.verticalStabilizer;
conf.em_hog.wingPhrase = conf.em_hog.verticalStabilizer;
% aeroplane
conf.em_hog.aeroplane = conf.em_hog.verticalStabilizer;
conf.em_hog.aeroplane.tpt_size = 30;
conf.em_hog.aeroplane.x_space = -1:1;
conf.em_hog.aeroplane.y_space = -1:1;
conf.em_hog.aeroplane.s_space = [0.8 0.7];
